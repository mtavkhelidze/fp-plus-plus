...
TEST(Kernel_Ops_Lift, simple_lambda) {
    auto doubleFn = lift<StructApply>([](int x) { return x * 2; });
    auto result = doubleFn(21);
    static_assert(std::is_same_v<decltype(result), StructApply<int>>);
    EXPECT_EQ(result.value(), 42);
}

TEST(Kernel_Ops_Lift, identity) {
    auto idFn = lift<StructApply>([](int x) { return x; });
    auto result = idFn(10);
...
TEST(Kernel_Ops_Lift, string_length) {
    auto lenFn =
      lift<StructApply>([](const std::string_view& s) { return s.length(); });
...
TEST(Kernel_Ops_Lift, type_change) {
    // A -> B where A != B
    auto toStringFn =
      lift<StructApply>([](int x) { return std::to_string(x); });
...
TEST(Kernel_Ops_Lift, nothing_result) {
    auto toNothing = lift<StructApply>([](int) { return nothing; });
    auto result = toNothing(42);
    static_assert(std::is_same_v<decltype(result), StructApply<Nothing>>);
...
TEST(Kernel_Ops_Lift, function_pointer) {
    auto fn =
      lift<StructApply>(static_cast<int (*)(int)>([](int x) { return x + 1; }));
    auto result = fn(41);
    static_assert(std::is_same_v<decltype(result), StructApply<int>>);
    EXPECT_EQ(result.value(), 42);
}
