...
TEST(Prelude_compose, works_with_multiple_compositions_right_to_left) {
    auto inittial = 2;
...
TEST(Prelude_compose, works_with_identity_function) {
...
TEST(Prelude_compose, works_with_nested_lambdas) {
    auto add_then_multiply = make_multiplier(3) * increment;

    EXPECT_EQ(add_then_multiply(2), 9);  // (2 + 1) * 3 = 9
}

TEST(Prelude_compose, forwards_arguments_correctly) {
...
TEST(Prelude_compose, composed_function_is_noexcept_if_parts_are_noexcept) {
...
TEST(Prelude_compose, works_with_structs_and_pairs) {
...
TEST(Prelude_compose, works_with_custom_functions_objects) {
